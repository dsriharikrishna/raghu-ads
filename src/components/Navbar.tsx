    ...
    useEffect(() => {
        ...
        if (raghuRef.current) {
            ...
            gsap.fromTo(letters,
                ...
                { opacity: 1, x: 0, skewX: 0, stagger: 0.06, duration: 0.5, ease: "back.out(2)", delay: 2.3 }
            );
        }
        if (adsRef.current) {
            ...
            gsap.fromTo(letters,
                ...
                { opacity: 1, scale: 1, rotation: 0, stagger: 0.08, duration: 0.5, ease: "elastic.out(1, 0.5)", delay: 2.65 }
            );
        }
        if (taglineRef.current) {
            gsap.fromTo(taglineRef.current,
                { opacity: 0, clipPath: "inset(0 100% 0 0)", letterSpacing: "0.4em" },
                { opacity: 1, clipPath: "inset(0 0% 0 0)", letterSpacing: "0.15em", duration: 0.8, ease: "power3.out", delay: 3.1 }
            );
        }
    }, []);
    ...
    return (
        <div className="flex flex-col leading-none select-none">
            <div className="flex items-baseline gap-1.5">
                <span ref={raghuRef} className="font-display text-[22px] text-white tracking-[0.08em] inline-flex">
                    {"RAGHU".split("").map((l, i) => <span key={i} className="letter inline-block opacity-0">{l}</span>)}
                </span>
                <span ref={adsRef} className="font-display text-[26px] tracking-[0.05em] inline-flex"
                    style={{ color: "#FFD600", WebkitTextStroke: "0.5px #c9a900" }}>
                    {"ADS".split("").map((l, i) => <span key={i} className="letter inline-block opacity-0">{l}</span>)}
                </span>
            </div>
            <span ref={taglineRef}
    ...
    return (
        <>
            <motion.nav
                ...
                className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-black/95 backdrop-blur-md border-b border-[#FFD600]/20" : "bg-transparent"
                    }`}
            >
                    ...
                    <div className="flex items-center justify-between h-16 md:h-20">
                        ...
                        <div className="flex items-center gap-3">
                            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                                ...
                                className="hidden md:flex items-center gap-2 bg-[#FFD600] text-black font-bold px-5 py-2.5 rounded-lg text-sm">
                                Get Quote
                            </motion.button>
            ...
            <AnimatePresence>
                {mobileOpen && (
                    <motion.div initial={{ opacity: 0, x: "100%" }} animate={{ opacity: 1, x: 0 }}
                        exit={{ opacity: 0, x: "100%" }} transition={{ type: "tween", duration: 0.32 }}
                        ...
                        <div className="flex flex-col gap-1">
                            {navLinks.map((link, i) => (
                                <motion.a key={link.href}
                                    ...
                                    href={link.href} onClick={(e) => { e.preventDefault(); handleNav(link.href); }}
